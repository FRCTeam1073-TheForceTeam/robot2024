...
public class CollectorTeleop extends Command {
  ...
  public void execute() {
    ...
    if(m_collector.getRange1() < minRange){
      m_collector.setCollectorSpeed(0);
    }

    if(m_OI.getOperatorRawButton(5))
    {
      m_collector.setCollectorSpeed(3); //meters per sec
    }
    else if(m_OI.getOperatorRawButton(6))
    {
      if (m_collector.getRange1() < minRange){
        m_collector.setCollectorSpeed(0); //meters per sec
      } else {
        double vel = m_drivetrain.getChassisSpeeds().vxMetersPerSecond + 1;
        m_collector.setCollectorSpeed(-vel); //meters per sec
      }
      }
